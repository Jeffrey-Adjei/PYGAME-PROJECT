...
class Game:
    def __init__(self, num_players, ai_style):
        ...
        self.play_for_me_button = Button(
            rect=(WIDTH // 2 - 300, HEIGHT - 100, 200, 50),
            color=(0, 200, 200),
            text="Play for Me",
            font=font_large,
    ...
    def ai_doing(self, player_index):
        ...
        if isinstance(player, AIPlayer):  # 检查玩家是否为 AI 类型
            ...
            self.action_log.add_log(f"{player.name} completed their turn with actions")
    ...
    def play_for_me(self):
        ...
        if isinstance(player, Hand):
            # 创建游戏状态供AI使用
            game_state = {
                'players': [p for i, p in enumerate(self.players) if i != self.current_turn],
                'deck': self.deck
            }
            # 临时AI对象，用于代替玩家执行操作
            temp_ai = AIPlayer(player.name, player.x, player.y)
            temp_ai.cards = player.cards[:]  # 复制玩家手牌
            temp_ai.actions_taken = player.actions_taken

            # 调用AI决策
            temp_ai.choose_action(game_state)

            # 更新玩家状态
            player.cards = temp_ai.cards[:]
            player.actions_taken = temp_ai.actions_taken

            # 记录AI操作日志
            actions_log = f"AI play for {player.name}: "
            if temp_ai.actions_taken.get('draw', False):
                actions_log += "draw cardd"
            if temp_ai.actions_taken.get('steal', False):
                actions_log += "steal card"
            if 'discard' in temp_ai.actions_taken and temp_ai.actions_taken['discard']:
                discarded_cards = [str(card) for card in temp_ai.actions_taken['discard']]
                actions_log += f"discard{', '.join(discarded_cards)}；"
            if not any(temp_ai.actions_taken.values()):
                actions_log += "no action"

            print(actions_log)
            self.action_log.add_log(actions_log)  # 添加到日志窗口中
        ...
        while True:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                    ...
                    if self.current_turn == 0 and self.play_for_me_button.handle_click(event.pos):
                        print("Play for me")
                        self.play_for_me()
